...
import type { Dims, ExtractResult, FurnitureCategory } from '@/lib/types';
...
export async function POST(request: Request) {
  ...
  try {
    ...
    return Response.json({ error: 'Provide a valid product URL.' }, { status: 400 });
  }
  ...
  if (!hasAllDims(result.dims) && process.env.ANTHROPIC_API_KEY) {
    try {
